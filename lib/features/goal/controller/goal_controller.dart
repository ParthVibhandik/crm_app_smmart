...
class GoalController extends GetxController {
  ...
  Future<void> createGoal() async {
    ...
    if (response.status) {
      ...
      CustomSnackBar.error(
          errorList: [response.message ?? 'Failed to save goal']);
    }
  ...
  Future<void> loadGoals() async {
    ...
    if (responseModel.status) {
      staffsModel =
          StaffsModel.fromJson(jsonDecode(responseModel.responseJson));
    }
    isLoading = false;
    update();
  }
  ...
  Future<void> loadStaffGoals(String staffId) async {
    ...
    if (responseModel.status) {
      StaffGoalsModel model =
          StaffGoalsModel.fromJson(jsonDecode(responseModel.responseJson));
      if (model.goals != null) {
  ...
  Future<void> loadGoalDetails(String goalId) async {
    ...
    if (responseModel.status) {
      try {
        ...
        if (model.data != null) {
          Get.to(
              () => GoalDetailsScreen(goal: model.data!, goalTypes: goalTypes));
        } else {
      ...
      CustomSnackBar.error(
          errorList: [responseModel.message ?? 'Failed to load details']);
    }
  }
