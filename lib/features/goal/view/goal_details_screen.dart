...
class GoalDetailsScreen extends StatelessWidget {
          ...
          children: [
            _buildSectionCard(
              ...
              children: [
                ...
                _buildDetailRow(context, 'Goal Type',
                    goalTypes[goal.goalType] ?? goal.goalType ?? '-'),
                const SizedBox(height: Dimensions.space15),
                Row(
                  children: [
                    Expanded(
                        child: _buildDetailRow(
                            context, 'Start Date', goal.startDate ?? '-')),
                    Expanded(
                        child: _buildDetailRow(
                            context, 'End Date', goal.endDate ?? '-')),
                  ],
                ),
                const SizedBox(height: Dimensions.space15),
                Row(
                  children: [
                    Expanded(
                        child: _buildDetailRow(
                            context, 'Target', goal.target ?? '0')),
                    Expanded(
                        child: _buildDetailRow(context, 'Achievement',
                            goal.achievement?.total?.toString() ?? '0')),
                  ],
                ),
              ],
            ),
  ...
  Widget _buildAchievementTab(BuildContext context) {
    // Parse values safely
    double percentStrValue =
        double.tryParse(goal.achievement?.percent ?? '0') ?? 0.0;
    ...
    double achievementAmount =
        double.tryParse(goal.achievement?.total?.toString() ?? '0') ?? 0.0;
                  ...
                  SizedBox(
                    ...
                    child: CircularProgressIndicator(
                      ...
                      valueColor: AlwaysStoppedAnimation<Color>(
                          _getColorForPercent(percentValue)),
                    ),
                  ),
                  Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: boldExtraLarge.copyWith(
                            fontSize: 40,
                            color: _getColorForPercent(percentValue)),
                      ),
                      Text(
                        "Completed",
                        style: lightDefault,
              ...
              _buildSectionCard(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      _buildStatItem(context, 'Total Achievement',
                          achievementAmount.toStringAsFixed(0)),
                      _buildStatItem(
                          context, 'Target', targetAmount.toStringAsFixed(0)),
                    ],
                  ),
                  const SizedBox(height: Dimensions.space20),
                  Center(
                      child: _buildStatItem(context, 'Left To Achieve',
                          leftAmount.toStringAsFixed(0),
      ...
      children: [
        Text(value,
            style: boldLarge.copyWith(
                fontSize: 24, color: color ?? ColorResources.primaryColor)),
