              ...
              children: [
                ...
                CustomDropDownTextField(
                  ...
                  items: controller.goalTypes.entries.map((e) {
                    return DropdownMenuItem(
                      value: e.key,
                      child: Text(e.value, style: regularDefault.copyWith(color: Colors.black)),
                    );
                  }).toList(),
                ),
                const SizedBox(height: Dimensions.space15),
                CustomDropDownTextField(
                  ...
                  items: controller.staffsModel.data?.map((staff) {
                    return DropdownMenuItem(
                      value: staff.id,
                      child: Text("${staff.firstName} ${staff.lastName}", style: regularDefault.copyWith(color: Colors.black)),
                    );
                  }).toList() ?? [],
                ),
                const SizedBox(height: Dimensions.space15),
                    ...
                    Expanded(
                      child: InkWell(
                        onTap: () async {
                          ...
                          if(pickedDate != null ){
                              controller.startDateController.text = DateConverter.formatDate(pickedDate);
                          }
                        },
                    ...
                    Expanded(
                      child: InkWell(
                         onTap: () async {
                          ...
                          if(pickedDate != null ){
                              controller.endDateController.text = DateConverter.formatDate(pickedDate);
                          }
                        },
                        child: CustomTextField(
                          labelText: "End Date",
